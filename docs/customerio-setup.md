...
| name | conditions (AND) |
|---|---|
| `consent` | attribute `marketing_consent` is `true` AND `email` exists |
| `signed_up_no_trade` | `email_bound` = true AND `campaign_trade_days` = 0 AND `megapot_churned` is not `true` |
...
For every campaign: **Trigger** as below, **Filter = `consent`**, **Goal/exit = event `megapot_trade`**
(except C which never exits), frequency "one at a time", re-entry off unless stated.

| id | type | trigger | delay | template | extra exit |
|---|---|---|---|---|---|
| A | segment | enters `signed_up_no_trade` | A1 +24h → A2 +24h → A3 +24h → A4 +2d → A5 +2d | A1..A5 | `megapot_hub_viewed` in last 12h skips A1; A4 step = **webhook** to ops (see 4) |
...
| C1 | event | `megapot_tickets_minted` | 0 | C1 | – |
| C2 | segment (recurring) | in `in_tonights_draw`, daily 15:00 UTC | 0 | C2 | once per day (dedupe on `megapot_status_at`) |
| C3 | event | `megapot_win_unclaimed` | 0 | C3 | – |
| C4 | segment | in `has_unclaimed` for 24h | 24h | C4 | exit on `megapot_win_claimed` |
| C5 | event | `megapot_win_claimed` | 0 | C5 | – |
...
Test with a whitelisted wallet profile (attributes are live after one engine cycle).
...
- Insights worth pinning: funnel `megapot_email_bound → megapot_activation_pack → megapot_trade
  (day 2)`; trend `megapot_streak_box` won vs empty by day; `cio_email_clicked` → `megapot_trade`
